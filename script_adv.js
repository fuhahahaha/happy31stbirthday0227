...
function startQ2Adventure() {
    // BGM切り替え: birthday停止 → umamusume 開始
    if (window.birthdayBgm) { window.birthdayBgm.pause(); window.birthdayBgm.currentTime = 0; }
    if (window.umaBgm) window.umaBgm.play().catch(() => {});
...
function onQ2Clear() {
    startHackerLoading(() => {
        ...
        document.getElementById('q3-screen').style.display = 'block';
    });
}
...
// Q3画面への遷移（ハッカー演出を挟むように修正）
function transitionToQ3() {
    // Q2を即座に隠す
    document.getElementById('q2-screen').style.display = 'none';

    // ハッカー演出開始
    startHackerLoading(() => {
        const q3 = document.getElementById('q3-screen');
        q3.classList.remove('hidden');
        // display: flex はCSSの :not(.hidden) で制御されるからそのままでOK！

        // BGM切り替え: umamusume停止 → kaiju 開始
        if (window.umaBgm) { window.umaBgm.pause(); window.umaBgm.currentTime = 0; }
        if (!window.kaijuBgm) { window.kaijuBgm = new Audio('bgm/kaiju.mp3'); window.kaijuBgm.loop = true; }
        window.kaijuBgm.play().catch(() => {});
    });
}
